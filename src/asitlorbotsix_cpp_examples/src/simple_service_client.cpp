...
public:
    ...
    {
        client_ = create_client<asitlorbot_msgs::srv::AddTwoInts>("add_two_ints");

        auto request = std::make_shared<asitlorbot_msgs::srv::AddTwoInts::Request>();
        request->a = a;
        request->b = b;
...
private:
    rclcpp::Client<asitlorbot_msgs::srv::AddTwoInts>::SharedPtr client_;

    void responseCallback(rclcpp::Client<asitlorbot_msgs::srv::AddTwoInts>::SharedFuture future)
    {
        if(future.valid())
        {
